...
func main() {
	...
	headlessChrome.ChromePath = `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`
	...
	// make a new session
	browser, err := headlessChrome.NewBrowser(`https://baidu.com`)
	if err != nil {
		panic(err)
	}
	...
	time.Sleep(headlessChrome.BrowserStartupTime + time.Second*5)
